...
class Predictor(torch.nn.Module):
	...
	def forward(self, sents_emb, mask):#, meta_inf):
		...
		out = self.linear_final(self.sigmoid(sents_matrix+docs_matrix))#+meta_matrix))
...
class InformationExtractor:
	def __init__(self):
		self.predictor = oie_predictor.from_path("https://s3-us-west-2.amazonaws.com/allennlp/models/openie-model.2018-08-20.tar.gz")
...
class Summarizer(nn.Module):
	def __init__(self,logger = None, coreference = False, extraction = False, no_bert = False, finetune = False, tfidf = False, length = False, relpos = False, pos = False):
	...
	def safe_model(self, folder, filename):
		if not os.path.exists(f'{folder}config.json'):
			with open(f'{folder}config.json', 'w') as fp:
				...
				args = {'No-BERT': self.no_bert, 'Fine-tune': self.finetune, 'Coreference': self.coreference, 'Extraction': self.extraction, 
						'Position': self.pos, 'Relative position': self.relpos, 'Length': self.length, 'TF_IDF': self.tfidf}
	...
	def load_model(self, reg_path, bert_path):
		try:
			if torch.cuda.is_available():
				...
				self.regression.load_state_dict(torch.load(reg_path, map_location='cpu'))
	...
	def from_pretrained(cls, folder, reg_file, bert_file = None, logger = None):
		try:
			with open(f'{folder}config.json', 'r') as config:
				...
				model = cls(logger = logger, coreference = args['Coreference'], extraction = args['Extraction'], no_bert = args['No-BERT'], 
					tfidf = args['TF_IDF'], length = args['Length'], relpos = args['Relative position'], pos = args['Position'])
	...
	def calculate_embeddings(self,sentence):
		...
		for line in lines:
			if len(line) > 512:
				line = line[:512]
			assert len(line) <= 512
			indexed_tokens = self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(line))
	...
	def build_features(self,text):
		...
		if self.no_bert:
			embeddings = [self.calculate_embeddings_(sent) for sent in tokenized_sentences]
		else: 
	...
	def build_labels(self,texts, summary): 
		...
		for doc in docs:
			...
			rouge_scores += [self.calculate_rouge(sent,summary, method = 'recall') for sent in sentences]
		if torch.cuda.is_available():
	...
	def sentence_relevance(self, sentences, summary_sentences):
		return [self.calculate_rouge(sent," ".join(summary_sentences), method = 'recall') for sent in sentences]
	...
	def sentence_features(self,text):
		...
		for i,sent in enumerate(tokenized_sentences):
			...
			tokenized_words_sent = [word for word in tokenized_words_with_stopwords_sent if word not in self.Stopwords]
			tokenized_words_sent = [word.lower() for word in tokenized_words_sent if len(word) > 1]
		...
		if self.relpos:
			rel_position = [(num_sent-i)/num_sent for i,sent in enumerate(tokenized_sentences)]
		...
		if self.tfidf:
			tfidf_scores = [self.calculate_tfids(sent,word_freq,tokenized_sentences) for i,sent in enumerate(tokenized_sentences)]
	...
	def idf_score(self,no_of_sentences,word,sentences):
		...
		for sentence in sentences:
			...
			sentence = [word for word in sentence if word.lower() not in self.Stopwords and len(word)>1]
	...
	def calculate_tfids(self,sentence,dict_freq,sentences):
		...
		for word in pos_tagged_sentence:
			if word.lower() not in self.Stopwords and word not in self.Stopwords and len(word)>1: 
				word = word.lower()
				word = self.wordlemmatizer.lemmatize(word)
				sentence_score = sentence_score + self.word_tfidf(dict_freq,word,sentences,sentence)
